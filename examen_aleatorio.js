        ...
        function comprobar() { // comprueba el numero de aciertos, inhabilita los radios y cambia el color a las respuetas correctas (a verde) y las incorrectas (a rojo)
            ...
            for (x = 0; x<inp_marcados.length;x+=3){ // for para cambiar el color e inhabilitar los radios
                for (j=0; j<3; j++){
                    resp_marcadas[x+j].classList.remove("lbl");
                    if (resp_pos_correcta[z]==j+1){
                        resp_marcadas[x+j].classList.add("valid");
                    }
                    else{
                        resp_marcadas[x+j].classList.add("invalid");
                    }
                    inp_marcados[x+j].disabled = true;
                }
                z++;
            }
